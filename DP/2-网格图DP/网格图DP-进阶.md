    ...
    def pathsWithMaxScore(self, board: List[str]) -> List[int]:
        ...
        for i in range(n):
            for j in range(n):
                ...
                if dp[i][j] != -1:
                    dp[i][j] += int(board[i][j]) if board[i][j].isdigit() else 0
        if dp[n-1][n-1] == -1:
            return [0, 0]
        # 再来个dp统计路径数
        ...
        for i in range(n):
            for j in range(n):
                if board[i][j] == 'X':
                    continue
                if i > 0 and dp[i-1][j] == dp[i][j] - (int(board[i][j]) if board[i][j].isdigit() else 0):
                    count[i][j] = (count[i][j] + count[i-1][j]) % MOD
                if j > 0 and dp[i][j-1] == dp[i][j] - (int(board[i][j]) if board[i][j].isdigit() else 0):
                    count[i][j] = (count[i][j] + count[i][j-1]) % MOD
                if i > 0 and j > 0 and dp[i-1][j-1] == dp[i][j] - (int(board[i][j]) if board[i][j].isdigit() else 0):
