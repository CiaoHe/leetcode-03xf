    ...
    def maximumProfit(self, prices: List[int], k: int) -> int:
        ...
        def dfs(i: int, j: int, end_state: int) -> int:
            if j<0:
                return -inf
            if i<0:
                return 0 if end_state==0 else -inf
            p = prices[i]
            if end_state == 0:
                return max(
                    dfs(i-1, j, 0), # 不交易
            ...
            if end_state == 2:
                return max(
                    dfs(i-1, j, 2), # 不交易, 维持做空状态
                    dfs(i-1, j-1, 0) + p, # 做空买入
                )
        ans = dfs(n-1, k, 0)
        dfs.cache_clear()
        return ans
        ...
        def dfs(i: int, j: int, end_state: int) -> int:
            ...
            if end_state==0:
                ...
                if j<k:
                    res = max(
                        res,
                        dfs(i+1, j, 1) - prices[i], # 买入(开多)
                        dfs(i+1, j, 2) + prices[i], # 做空(开空)
                    )
                return res
            if end_state==1:
                return max(
                    dfs(i+1, j, 1), # 不交易 继续持多
                    dfs(i+1, j+1, 0) + prices[i], # 卖出平仓，交易次数+1
                )
            if end_state==2:
                return max(
                    dfs(i+1, j, 2), # 不交易 继续持空
                    dfs(i+1, j+1, 0) - prices[i], # 买入并平仓，交易次数+1
                )
        ans = dfs(0, 0, 0)
        dfs.cache_clear()
        return ans
