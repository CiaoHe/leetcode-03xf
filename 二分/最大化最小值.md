- **找最后一个满足 check(x) 为 True 的值**​ → 这是“最大化最小值”、“最大化某个指标”的典型场景。
    等价于：在有序序列中找最后一个 ≤ target 的元素 → 对应 `bisect_right(list, target) - 1
- **找第一个满足 check(x) 为 True 的值**​ → 这是“最小化最大值”、“最小化某个指标”的典型场景。
    等价于：在有序序列中找第一个 ≥ target 的元素 → 对应 `bisect_left(list, target)`。
# [1552. 两球之间的磁力](https://leetcode.cn/problems/magnetic-force-between-two-balls/)
```python
class Solution:
    def maxDistance(self, position: List[int], m: int) -> int:
        # 二分搜索
        position.sort()
        n = len(position)
        l, r = 1, position[-1] - position[0]
        def check(mid):
            # Check if there are m positions such that the distance between adjacent positions is at least mid
            cnt = 1  # Start with the first position counted
            last_position = position[0]  # The position of the last counted position
            for i in range(1, n):
                if position[i] - last_position >= mid:
                    cnt += 1
                    if cnt >= m:
	                    return True
                    last_position = position[i]  # Update the last counted position
            return cnt >= m
        while l < r:
            mid = (l + r + 1) // 2
            if check(mid):
                l = mid
            else:
                r = mid - 1
        return l

```
# [2528. 最大化城市的最小电量](https://leetcode.cn/problems/maximize-the-minimum-powered-city/)
给你一个下标从 **0** 开始长度为 `n` 的整数数组 `stations` ，其中 `stations[i]` 表示第 `i` 座城市的供电站数目。

每个供电站可以在一定 **范围** 内给所有城市提供电力。换句话说，如果给定的范围是 `r` ，在城市 `i` 处的供电站可以给所有满足 `|i - j| <= r` 且 `0 <= i, j <= n - 1` 的城市 `j` 供电。

- `|x|` 表示 `x` 的 **绝对值** 。比方说，`|7 - 5| = 2` ，`|3 - 10| = 7` 。

一座城市的 **电量** 是所有能给它供电的供电站数目。

政府批准了可以额外建造 `k` 座供电站，你需要决定这些供电站分别应该建在哪里，这些供电站与已经存在的供电站有相同的供电范围。

给你两个整数 `r` 和 `k` ，如果以最优策略建造额外的发电站，返回所有城市中，最小电量的最大值是多少。

这 `k` 座供电站可以建在多个城市。


```python
class Solution:
    def maxPower(self, stations: List[int], r: int, k: int) -> int:
        n = len(stations)
        diff = [0] * (n + 1) # 差分数组
        for i,v in enumerate(stations):
            # 确定窗口
            left = max(0, i - r) 
            right = min(n - 1, i + r)
            diff[left] += v
            diff[right + 1] -= v
        s = list(accumulate(diff)) # 前缀和：表示当前城市access的发电站数目
        left, right = 0, 1<<40
        
        def check(x, k):
            # 检查在build最多k个站点后，是否可以使得每个城市的电力都>=x
            # 贪心，从左到右，每次都把当前位置的站点建到最大
            d = [0] * (n+1) # 差分数组，记录新增的发电站
            t = 0 # 当前累计的新增站点数
            for i in range(n):
                t += d[i]
                dist = x - (s[i] + t) # 当前城市需要建的站点数
                if dist > 0:
                    if k < dist:
                        # 不满足
                        return False
                        
                    k -= dist
                    # 在当前位置建设dist个发电站
                    # 这些发电站会影响从 i 到 i+2r 范围内的城市
                    start = i  # 影响范围的起点
                    end = min(n-1, i + 2*r)  # 影响范围的终点
                    
                    d[start] += dist
                    if end + 1 < n:
                        d[end + 1] -= dist
                    t += dist  # 更新当前累计新增的电力
            return True
        
        while left < right:
            mid = (left + right + 1) // 2
            if check(mid, k):
                left = mid
            else:
                right = mid - 1
        return left
```

# [3464. 正方形上的点之间的最大距离](https://leetcode.cn/problems/maximize-the-distance-between-points-on-a-square/)
给你一个整数 `side`，表示一个正方形的边长，正方形的四个角分别位于笛卡尔平面的 `(0, 0)` ，`(0, side)` ，`(side, 0)` 和 `(side, side)` 处。

同时给你一个 **正整数** `k` 和一个二维整数数组 `points`，其中 `points[i] = [xi, yi]` 表示一个点在正方形**边界**上的坐标。

你需要从 `points` 中选择 `k` 个元素，使得任意两个点之间的 **最小** 曼哈顿距离 **最大化** 。

返回选定的 `k` 个点之间的 **最小** 曼哈顿距离的 **最大** 可能值。

两个点 `(xi, yi)` 和 `(xj, yj)` 之间的曼哈顿距离为 `|xi - xj| + |yi - yj|`。

```python
class Solution:
    def maxDistance(self, side: int, points: List[List[int]], k: int) -> int:
        # 最大化最小值 -> 二分

        # 把正方形上的点 按照左上右下的顺序排序
        a = []
        for x,y in points:
            if x == 0:
                a.append(y)
            elif y == side: # 上
                a.append(side + x)
            elif x == side: # 右
                a.append(2*side + (side - y))
            else: # 下
                a.append(3*side + (side - x))
        a.sort()

        def check(low: int) -> bool:
            # 判断是否存在k个点, 使得任意两个点的距离都>=low

            # 问题变成：能否在a里面找到k个数 要求任意两个数的差都>=low 且第一个数和最后一个数的差<=4*side-low
            ## 假设第一个点x 第二点y 那么x和y的距离是min(y-x, 4*side-(y-x)) 因此要求y-x>=low 且 4*side-(y-x)>=low 即 y-x<=4*side-low
            for start in a:
                end = start + 4*side - low
                p = start
                for _ in range(k-1): # 还需要找到k-1个点
                    # 在a里面找到第一个>=p+low的数
                    i = bisect_left(a, p + low)
                    if i == len(a) or a[i] > end:
                        break
                    p = a[i]
                else:
                    return True
            return False
        
        left, right = 1, side+1
        while left < right:
            mid = (left + right) // 2
            if check(mid):
                left = mid + 1
            else:
                right = mid
        return left - 1

```

# [3620. 恢复网络路径](https://leetcode.cn/problems/network-recovery-pathways/)
给你一个包含 `n` 个节点（编号从 0 到 `n - 1`）的有向无环图。图由长度为 `m` 的二维数组 `edges` 表示，其中 `edges[i] = [ui, vi, costi]` 表示从节点 `ui` 到节点 `vi` 的单向通信，恢复成本为 `costi`。

一些节点可能处于离线状态。给定一个布尔数组 `online`，其中 `online[i] = true` 表示节点 `i` 在线。节点 0 和 `n - 1` 始终在线。

从 0 到 `n - 1` 的路径如果满足以下条件，那么它是 **有效** 的：

- 路径上的所有中间节点都在线。
- 路径上所有边的总恢复成本不超过 `k`。

对于每条有效路径，其 **分数** 定义为该路径上的最小边成本。

返回所有有效路径中的 **最大** 路径分数（即最大 **最小** 边成本）。如果没有有效路径，则返回 -1。

```python
class Solution:
    def findMaxPathScore(self, edges: List[List[int]], online: List[bool], k: int) -> int:
        n = len(online)
        g = [[] for _ in range(n)]
        max_wt = -1 # max_wt代表从0出发到达n-1的边的最大权重
        for u, v, w in edges:
            if online[u] and online[v]:
                g[u].append((v, w))
                if u == 0:
                    max_wt = max(max_wt, w)
        
        # 二分去找到最小的max_wt，使得从0出发到达n-1的路径长度不超过k
        def check(lower:int) -> bool:
            # lower表示当前的max_wt
            # 如果从0出发到达n-1的路径长度不超过k，则返回True，否则返回False
            
            # 我们用记忆化搜索, dfs(x)表示从x出发到达n-1的路径长度
            @cache
            def dfs(x:int)->int:
                if x == n-1:
                    return 0
                res = inf
                for y, w in g[x]:
                    if w >= lower: # 可以加入
                        res = min(res, dfs(y) + w)
                return res
            return dfs(0)<=k

        left, right = -1, max_wt+1 # 开区间写法, 不需要考虑+1/-1
        while left + 1 < right: # 注意开区间时候拉开
            mid = (left + right) // 2
            if check(mid):
                left = mid # left始终指向一个满足​ check 的值
            else:
                right = mid # right始终指向一个不满足​ check 的值
        return left
```