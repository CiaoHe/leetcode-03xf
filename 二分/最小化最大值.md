...
1. **确定边界**：
    - **左边界 `low`**: $0$ 秒。
    - **右边界 `high`**: 找一个最慢的工人独自干完所有活的时间。假设最慢时间为 $max(workerTimes)$，高度为 $H$，则最高耗时约为 $base \times \frac{H(H+1)}{2}$。
2. **二分搜索**：
    - 取中间时间 `mid`。
    - 计算在 `mid` 秒内，每个工人 $i$ 能降低的最大高度 $x_i$。
    - 如果 $\sum x_i \ge mountainHeight$，说明时间足够，甚至多了，我们可以尝试缩短时间（`high = mid`）。
    - 否则，说明时间不够，需要增加时间（`low = mid + 1`）。
        如何计算 $x_i$？

对于工人 $i$，在 $t$ 秒内能达到的最大高度 $x$ 满足：

$$workerTimes[i] \times \frac{x(x+1)}{2} \le t$$

整理得：$x^2 + x - \frac{2t}{workerTimes[i]} \le 0$。

我们可以直接用 **求根公式** 或者在内部嵌套一个**小二分**来求出 $x$。
...
class Solution:
    def minNumberOfSeconds(self, mountainHeight: int, workerTimes: List[int]) -> int:
        ...
        low = 0
        max_base = min(workerTimes) # 用最快的人估算上限更紧凑
        high = max_base * mountainHeight * (mountainHeight + 1) // 2
        ans = high
        while low <= high:
            mid = (low + high) // 2
            if check(mid):
