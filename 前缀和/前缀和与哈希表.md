通常要用到「枚举右，维护左」的技巧。
# [2588. 统计美丽子数组数目](https://leetcode.cn/problems/count-the-number-of-beautiful-subarrays/)
实际上A subarray is beautiful if its xor is equal to zero.
1. 计算出prefix xor
2. 如果`prefix[left] = prefix[right]`, 那么`[left, right] `这段beautiful
```python
class Solution:
    def beautifulSubarrays(self, nums: List[int]) -> int:
        # A subarray is beautiful if its xor is equal to zero.
        # compute the prefix xor
        prefix_xor = [0]
        for num in nums:
            prefix_xor.append(prefix_xor[-1] ^ num)
        # count the frequency of each prefix xor
        freq = Counter(prefix_xor)
        res = 0
        for k, v in freq.items():
            res += v * (v - 1) // 2
        return res
```

# [1590. 使数组和能被 P 整除](https://leetcode.cn/problems/make-sum-divisible-by-p/)
给你一个正整数数组 `nums`，请你移除 **最短** 子数组（可以为 **空**），使得剩余元素的 **和** 能被 `p` 整除。 **不允许** 将整个数组都移除。

请你返回你需要移除的最短子数组的长度，如果无法满足题目要求，返回 `-1` 。

**子数组** 定义为原数组中连续的一组元素。

> 输入：nums = [3,1,4,2], p = 6
> 输出：1
> 解释：nums 中元素和为 10，不能被 p 整除。我们可以移除子数组 [4] ，剩余元素的和为 6 

```python
class Solution:
    def minSubarray(self, nums: List[int], p: int) -> int:
        k = sum(nums) % p # 目标前缀和%p的值
        if k == 0:
            return 0
        last = {0: -1} # 当前前缀和%p的值，对应的最后一个位置
        ans = len(nums)
        cur = 0
        for i, x in enumerate(nums):
            # 当前包含x的前缀和%p的值
            cur = (cur + x) % p
            if (cur - k) % p in last:
                ans = min(ans, i - last[(cur - k) % p]) # [last, i-1]的子数组和%p的值为k都需要删除, 长度为i-last
            # update last
            last[cur] = i
        return ans if ans < len(nums) else -1
```

# [525. 连续数组](https://leetcode.cn/problems/contiguous-array/)
给定一个二进制数组 `nums` , 找到含有相同数量的 `0` 和 `1` 的最长连续子数组，并返回该子数组的长度。

```python
class Solution:
    def findMaxLength(self, nums: List[int]) -> int:
        n = len(nums)
        ans = 0
        # 遇到奇数，+1，遇到偶数，-1
        diff = 0
        diff_to_idx = {0: -1}
        for i in range(n):
            if nums[i] % 2 == 0:
                diff -= 1
            else:
                diff += 1
            
            # 其实当前diff和历史中上一个diff位置中间 这一块 也是平衡的
            # 不一定非得是diff=0才能更新，diff=0指的是从头到尾都是平衡的
            if diff in diff_to_idx:
                ans = max(ans, i - diff_to_idx[diff])
            else:
                diff_to_idx[diff] = i
        return ans
```

# [3714. 最长的平衡子串 II](https://leetcode.cn/problems/longest-balanced-substring-ii/)
给你一个只包含字符 `'a'`、`'b'` 和 `'c'` 的字符串 `s`。
如果一个 **子串** 中所有 **不同** 字符出现的次数都 **相同**，则称该子串为 **平衡** 子串。
请返回 `s` 的 **最长平衡子串** 的 **长度** 。
**子串** 是字符串中连续的、**非空** 的字符序列。

```python
class Solution:
    def longestBalanced(self, s: str) -> int:
        n = len(s)
        ans = 0

        # 包含三种字符a b c
        # (count_a - count_b, count_b - count_c)
        seen = {(0, 0): -1}
        a = b = c = 0
        for i, ch in enumerate(s):
            if ch == 'a':
                a += 1
            elif ch == 'b':
                b += 1
            elif ch == 'c':
                c += 1
            key = (a - b, b - c)
            if key in seen:
                ans = max(ans, i - seen[key])
            else:
                seen[key] = i
        
        # 包含两种字符
        def check_two_chars(x:str, y:str, forbidden:str) -> int:
            nonlocal ans
            # 哈希表记录 (差值, 第一次索引)
            # diff = count(x) - count(y)
            mp = {0:-1}
            diff = 0

            for i, ch in enumerate(s):
                if ch == forbidden:
                    # reset, start from i
                    mp = {0:i}
                    diff = 0
                else:
                    if ch == x:
                        diff += 1
                    elif ch == y:
                        diff -= 1
                    if diff in mp:
                        ans = max(ans, i - mp[diff])
                    else:
                        mp[diff] = i
        
        check_two_chars('a', 'b', 'c')
        check_two_chars('a', 'c', 'b')
        check_two_chars('b', 'c', 'a')
        
        # 只包含1种字符
        cur_len = 0
        last_char = ''
        for ch in s:
            if ch == last_char:
                cur_len += 1
            else:
                last_char = ch
                cur_len = 1
            ans = max(ans, cur_len)
        
        return ans
```
# [3737. 统计主要元素子数组数目 I](https://leetcode.cn/problems/count-subarrays-with-majority-element-i/)
给你一个整数数组 `nums` 和一个整数 `target`。
返回数组 `nums` 中满足 `target` 是 **主要元素** 的 **子数组** 的数目。

一个子数组的 **主要元素** 是指该元素在该子数组中出现的次数 **严格大于** 其长度的 **一半** 。
**子数组** 是数组中的一段连续且 **非空** 的元素序列。

```python
class Solution:
    def countMajoritySubarrays(self, nums: List[int], target: int) -> int:
        n = len(nums)
        ans = 0
        # 把target设为1，其他数设为-1，求前缀和>0的个数
        pre = [0] * (n + 1)
        for i in range(n):
            pre[i + 1] = pre[i] + (1 if nums[i] == target else -1)
        # 统计前缀和的个数
        for i in range(n):
            for j in range(i + 1, n + 1):
                if pre[j] - pre[i] > 0:
                    ans += 1
        return ans
```

# [3739. 统计主要元素子数组数目 II](https://leetcode.cn/problems/count-subarrays-with-majority-element-ii/)
[[3737. 统计主要元素子数组数目 I](https://leetcode.cn/problems/count-subarrays-with-majority-element-i/)](#[3737.%20统计主要元素子数组数目%20I](https%20//leetcode.cn/problems/count-subarrays-with-majority-element-i/)) 时间卡的更紧

```python
class Solution:
    def countMajoritySubarrays(self, nums: List[int], target: int) -> int:
        n = len(nums)
        ans = 0
        # 把target设为1，其他数设为-1，求前缀和>0的个数
        # 我们假设到一个地方的presum是x，那么我们需要找到上一个小于他的presum的index
        # 我们可以用sortedlist来维护之前出现的所有前缀和
        presum = 0
        sl = SortedList([0])
        for x in nums:
            presum += 1 if x == target else -1
            # 返回有多少个已存在的前缀和严格小于当前的 prefix
            cnt = sl.bisect_left(presum)
            ans += cnt
            sl.add(presum)
        return ans
```